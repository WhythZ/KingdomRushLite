...
void StatusUI::OnRender(SDL_Renderer* _renderer)
{
	...
	//获取地图Rect（而不是窗口尺寸，因为地图可能与窗口大小不等）
	...
	{
		_positionLeftUp.x = _mapRect.x + _i * (healthHeartIconBetweenDistance + healthHeartIconSize.x);
	...
	//金币图标置于血量图标下方一定距离
	_positionLeftUp.x = _mapRect.x;
	_positionLeftUp.y = _mapRect.y + healthHeartIconSize.y + rowBetweenDistance;
	_ui->DrawTexture(_renderer, TextureResID::UI_Icon_Coin, _positionLeftUp, coinIconSize);
	#pragma endregion

	#pragma region CoinNumText
	//金币数量的实时更新文本，位置在金币图标右侧一定距离
	_positionLeftUp.x = _mapRect.x + coinIconSize.x + iconTextBetweenDistance;
	_positionLeftUp.y = _mapRect.y + healthHeartIconSize.y + rowBetweenDistance;
	...
	#pragma region WaveNumText
	//渲染在屏幕右上角
	_positionLeftUp.x = _mapRect.x + _mapRect.w - waveNumTextSize.x;
	_positionLeftUp.y = _mapRect.y;
	...
	//将玩家图标渲染在屏幕左下角
	_positionLeftUp.x = _mapRect.x;
	_positionLeftUp.y = _mapRect.y + _mapRect.h - playerIconSize.y;
	//获取当前玩家种类，绘制对应图标
	...
	_positionLeftUp.x = _mapRect.x + playerIconSize.x + iconBarBetweenDistance;
	_positionLeftUp.y = _mapRect.y + _mapRect.h - playerIconSize.y;
	...
	//第二个法力值条在第一个的下方
	_positionLeftUp.x = _mapRect.x + playerIconSize.x + iconBarBetweenDistance;
	_positionLeftUp.y = (_mapRect.y + _mapRect.h - playerIconSize.y) + mpBarSize.y + rowBetweenDistance;
	_ui->DrawDynamicBar(_renderer, _positionLeftUp, mpBarSize, mpBarBorderThickness,
